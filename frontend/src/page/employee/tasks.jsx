      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5">
          <div className="space-y-2">
            ...
            <textarea 
              ...
              className="w-full h-32 p-4 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-4 focus:ring-[#018790]/10 focus:border-[#018790] outline-none resize-none transition-all placeholder:text-slate-400 text-slate-700 font-medium"
            />
          </div>
          ...
          <div className="space-y-2">
            ...
            <div 
              ...
              className={`border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 group
                ${fileAttached ? 'border-emerald-400 bg-emerald-50' : 'border-slate-200 hover:border-[#018790] hover:bg-slate-50'}`}
            >
              {fileAttached ? (
                ...
                <>
                  <div className="w-12 h-12 bg-slate-100 group-hover:bg-[#018790]/10 rounded-full flex items-center justify-center mb-3 transition-colors">
                    <UploadCloud size={24} className="text-slate-400 group-hover:text-[#018790]" />
                  </div>
        ...
        <div className="p-5 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
          ...
          <button 
            ...
            className="px-6 py-2.5 bg-[#018790] text-white font-bold text-sm rounded-xl shadow-lg shadow-teal-900/20 hover:bg-[#006a70] active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
...
export default function EmployeeTaskPage() {
  ...
  const stats = [
    ...
    { label: 'En cours', count: inProgressTasks.length, color: 'text-[#018790]', bg: 'bg-[#018790]/10', icon: PlayCircle },
        ...
        {activeTab === 'todo' && (
          <div className="grid md:grid-cols-2 gap-6 animate-[fade-in_0.4s_ease-out]">
            {inProgressTasks.length === 0 ? <div className="col-span-2"><EmptyState message="Aucune tâche en cours." icon={ClipboardList} /></div> : inProgressTasks.map(task => (
                <SpotlightCard theme="light" key={task.id} className="flex flex-col h-full border-l-4 border-l-[#018790]">
                  <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity"><Clock size={120} className="text-[#018790]" /></div>
                  <div className="relative z-10 flex-1">
                    <div className="flex justify-between items-start mb-4">
                      <span className="px-3 py-1 bg-[#018790]/10 text-[#018790] rounded-lg text-[10px] font-bold uppercase tracking-wider">En cours</span>
                      <div className="animate-pulse w-2.5 h-2.5 bg-[#018790] rounded-full shadow-[0_0_8px_#018790]"></div>
                    </div>
                  ...
                  <button onClick={() => initiateFinishTask(task)} className="relative z-10 w-full py-3.5 bg-[#018790] text-white rounded-xl text-sm font-bold shadow-lg shadow-teal-900/20 hover:bg-[#006a70] active:scale-95 transition-all flex items-center justify-center gap-2">
