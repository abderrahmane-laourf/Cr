...
const ParcelCard = ({ parcel, onStatusChange, product }) => {
  ...
  return (
    <div className={`bg-white rounded-2xl p-4 shadow-sm border border-slate-100 mb-3 relative group transition-all ${
        parcel.stage === STAGE_LIVRE ? 'opacity-75' : ''
    }`}>
      {/* CARD HEADER / MAIN INFO */}
      <div className="flex gap-4" onClick={() => setExpanded(!expanded)}>
          {/* Image / Icon Placeholder */}
          <div className="w-14 h-14 rounded-2xl bg-slate-50 shrink-0 flex items-center justify-center overflow-hidden border border-slate-200">
             {product?.image ? (
          ...
          <div className="min-w-0 flex-1">
             <div className="flex justify-between items-start">
                 <h4 className="font-bold text-sm truncate text-slate-900 leading-tight">{product?.nom || 'Produit'}</h4>
                 <p className="text-xs font-black text-slate-700 whitespace-nowrap">{parcel.prix} DH</p>
             </div>
      ...
      {/* Status Badge (Absolute or placed) */}
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-50">
          <div className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide flex items-center gap-1 ${displayInfo.color}`}>
             {parcel.stage === STAGE_LIVRE ? <Check size={12}/> : parcel.stage === STAGE_REPORTE ? <Clock size={12}/> : <Truck size={12}/>}
             {displayInfo.label}
          </div>
          <button onClick={() => setExpanded(!expanded)} className="text-xs font-bold text-slate-400">
      ...
      {/* EXPANDED ACTIONS */}
      {expanded && (
        <div className="mt-4 pt-2 border-t border-slate-100 animate-in slide-in-from-top-2">
            
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 mb-4 px-1">
                <div className="flex items-center gap-2"><Phone size={14}/> {parcel.tel}</div>
                {parcel.commentaire && <div className="col-span-2 text-amber-600 flex items-center gap-1"><AlertCircle size={14}/> {parcel.commentaire}</div>}
            </div>

            {/* ACTION BUTTONS (Only if not already delivered/canceled, or allow changes?) */}
            {/* Allowing changes even if delivered to fix mistakes */}
            <div className="grid grid-cols-4 gap-2">
                <a href={`tel:${parcel.tel}`} className="flex flex-col items-center justify-center bg-blue-50 hover:bg-blue-100 text-blue-600 p-3 rounded-2xl transition-colors">
                    <Phone size={20} />
                    <span className="text-[10px] font-bold mt-1">Appel</span>
                </a>
                <button className="flex flex-col items-center justify-center bg-slate-50 hover:bg-slate-100 text-slate-600 p-3 rounded-2xl transition-colors">
                    <Navigation size={20} />
                    <span className="text-[10px] font-bold mt-1">GPS</span>
                </button>
                <button 
                    onClick={() => onStatusChange(parcel.id, STAGE_REPORTE)} 
                    className={`flex flex-col items-center justify-center p-3 rounded-2xl transition-colors ${parcel.stage === STAGE_REPORTE ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/30' : 'bg-amber-50 hover:bg-amber-100 text-amber-600'}`}
                >
                    <Clock size={20} />
                ...
                <button 
                    ...
                    className={`flex flex-col items-center justify-center p-3 rounded-2xl transition-colors ${parcel.stage === STAGE_LIVRE ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'bg-emerald-50 hover:bg-emerald-100 text-emerald-600'}`}
                >
                    <Check size={20} />
             ...
             <button 
                ...
                className="w-full mt-3 py-2 text-xs font-bold text-red-400 hover:text-red-500 flex items-center justify-center gap-1"
            >
  ...
  return (
    <div className="pb-24 max-w-lg mx-auto p-4 bg-slate-50 min-h-screen">
       ...
       <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
             <button onClick={() => setFilter('active')} className={`flex-1 min-w-[100px] py-3 text-xs font-bold rounded-2xl transition-all border ${filter === 'active' ? 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-500/30' : 'bg-white border-slate-200 text-slate-500'}`}>
                En Cours ({run.filter(r => [STAGE_EN_ROUTE, STAGE_PACKAGING].includes(r.stage)).length})
             </button>
             <button onClick={() => setFilter('delivered')} className={`flex-1 min-w-[100px] py-3 text-xs font-bold rounded-2xl transition-all border ${filter === 'delivered' ? 'bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-500/30' : 'bg-white border-slate-200 text-slate-500'}`}>
                Livrés ({run.filter(r => [STAGE_LIVRE].includes(r.stage)).length})
             </button>
             <button onClick={() => setFilter('exceptions')} className={`flex-1 min-w-[100px] py-3 text-xs font-bold rounded-2xl transition-all border ${filter === 'exceptions' ? 'bg-amber-500 border-amber-500 text-white shadow-lg shadow-amber-500/30' : 'bg-white border-slate-200 text-slate-500'}`}>
