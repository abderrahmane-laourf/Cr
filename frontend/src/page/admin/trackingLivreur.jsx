      ...
      <SpotlightCard theme="light" className="mb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <h1 className="text-3xl font-extrabold text-[#2563EB] flex items-center gap-3">
                <Navigation className="text-[#1e3a8a]" size={32} />
                Tracking du Livreur
            </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {livreurs.map(livreur => (
          <SpotlightCard
            key={livreur.id}
            theme="light"
            className="!p-0 overflow-hidden cursor-pointer group hover:border-[#2563EB]/50 transition-all"
            onClick={() => setSelectedLivreur(livreur)}
          >
            {/* Card Header */}
            <div className={`p-6 border-b border-slate-100 ${
              livreur.status === 'active' 
                ? 'bg-gradient-to-r from-[#1e3a8a]/5 to-[#2563EB]/5' 
                : 'bg-slate-50/50'
            }`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#1e3a8a] font-bold shadow-sm ring-2 ring-white">
                    {livreur.name.charAt(0)}
                  </div>
                  <div>
                    <h3 className="font-bold text-slate-800 text-lg group-hover:text-[#1e3a8a] transition-colors">{livreur.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
                  ...
                  <p className="text-2xl font-extrabold text-[#1e3a8a]">{livreur.assignedPackages}</p>
                </div>
                <div className="bg-white p-3 rounded-xl border border-slate-100 shadow-sm">
            ...
            <div className="p-6 space-y-4 bg-white">
              ...
              <div className="flex gap-3 pt-2">
                <a
                  ...
                  className="flex-1 py-2.5 bg-[#1e3a8a] text-white rounded-xl hover:bg-[#1e40af] transition-all flex items-center justify-center gap-2 text-sm font-bold shadow-md shadow-blue-900/20"
                >
                  <MapPin size={16} />
                  Localiser
                </a>
      ...
      {selectedLivreur && createPortal(
        ...
        >
          <SpotlightCard 
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-[#1e3a8a] text-white p-6 z-10">
            ...
            <div className="p-8 space-y-8 bg-white dark:bg-slate-900">
              ...
              <div className="space-y-3">
                 <h4 className="font-bold text-[#1e3a8a] flex items-center gap-2 text-sm uppercase tracking-wide">
                 ...
                 <div className="aspect-video bg-slate-100 rounded-2xl overflow-hidden border-2 border-slate-100 relative shadow-inner">
                    ...
                    ></iframe>
                    <div className="absolute top-4 left-4 bg-white/90 backdrop-blur p-2 rounded-lg shadow-sm text-xs font-bold text-[#1e3a8a] border border-[#1e3a8a]/10 flex items-center gap-2">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-[#1e3a8a]/5 p-4 rounded-2xl text-center border border-[#1e3a8a]/10">
                  <Package className="w-6 h-6 mx-auto mb-2 text-[#1e3a8a]" />
                  <p className="text-3xl font-extrabold text-[#1e3a8a]">{selectedLivreur.assignedPackages}</p>
              ...
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                <h4 className="font-bold text-[#1e3a8a] mb-4 text-sm uppercase tracking-wide">Informations Détaillées</h4>
