  ...
  return (
    <div className="fixed top-6 right-6 z-[60] animate-in slide-in-from-right-10 fade-in duration-300">
      <div className={`flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl border backdrop-blur-md
        ${type === 'success' ? 'bg-[#1e3a8a] text-white border-[#1e3a8a]' : 'bg-[#1e3a8a]/10 border-[#1e3a8a]/20 text-[#1e3a8a]'}`}>
        {type === 'success' ? <CheckCircle size={24} className="text-white" /> : <AlertCircle size={24} className="text-[#1e3a8a]" />}
        <div>
...
const TransactionModal = ({ isOpen, onClose, onSave, mode, employees }) => {
    ...
    const getHeaderColor = () => {
        ...
        return 'bg-[#1e3a8a]/10 text-[#1e3a8a]'; // Expense Virement
    };

    return createPortal(
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 border border-slate-100">
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="group">
                        ...
                        <input 
                            type="date"
                            required
                            className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 text-sm focus:bg-white focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10 outline-none"
                            value={formData.date}
                    ...
                    {mode === 'OUT_VIREMENT' && (
                        <div className="space-y-4 bg-[#1e3a8a]/5 p-5 rounded-2xl border border-[#1e3a8a]/10 animate-in slide-in-from-top-2">
                            <div className="group">
                                <label className="block text-xs font-bold text-[#1e3a8a] mb-1.5 ml-1 uppercase tracking-wider">1. Sélectionner l'Employé</label>
                                <select 
                                    required
                                    className="w-full px-4 py-3 rounded-xl bg-white border border-[#2563EB]/20 text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20"
                            ...
                            {formData.employeeId && (
                                <div className="group animate-in fade-in slide-in-from-top-1">
                                    <label className="block text-xs font-bold text-[#1e3a8a] mb-2 ml-1 uppercase tracking-wider">2. Choisir le Numéro</label>
                                    <div className="space-y-2">
                                        {availablePhones.length > 0 ? availablePhones.map((phone, idx) => (
                                            <label key={idx} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${formData.phoneNumber === phone ? 'bg-[#1e3a8a] border-[#1e3a8a] text-white shadow-md' : 'bg-white border-slate-200 text-slate-600 hover:border-[#2563EB]/50'}`}>
                                                <input 
                                                    type="radio" 
                                                    name="phone" 
                                                    required
                                                ...
                                                />
                                                <Smartphone size={18} className={formData.phoneNumber === phone ? 'text-white' : 'text-[#2563EB]'} />
                    ...
                    <div className="space-y-4">
                        <div className="group">
                            ...
                            <div className="relative">
                                    ...
                                    step="0.01"
                                    placeholder="0.00"
                                    className="w-full pl-4 pr-12 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 text-lg font-mono font-bold focus:bg-white focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10 outline-none transition-all"
                            ...
                            <textarea 
                                ...
                                className={`w-full px-4 py-3 rounded-xl border text-slate-700 text-sm outline-none transition-all resize-none
                                    ${(mode === 'OUT_VIREMENT') 
                                        ...
                                        : 'bg-slate-50 border-slate-200 focus:bg-white focus:border-[#2563EB] focus:ring-4 focus:ring-[#2563EB]/10'}`}
                    ...
                    <div className="pt-2">
                        <button 
                            ...
                            className={`w-full py-3.5 rounded-xl text-white font-bold flex items-center justify-center gap-2 shadow-lg transition-all transform active:scale-95
                                ...
                                ${isVirementContext && !isIncome ? 'bg-[#1e3a8a] hover:bg-[#1e40af] shadow-blue-900/30' : ''}
                            `}
                        >
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {virementTransactions.map(t => (
                                    <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className={`px-6 py-3 text-right font-mono font-bold text-sm ${t.type === 'IN' ? 'text-emerald-600' : 'text-[#2563EB]'}`}>
    ...
    return (
        <div className="w-full min-h-screen bg-transparent p-6 space-y-8 animate-[fade-in_0.6s_ease-out] dark:text-slate-200">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center bg-transparent p-6 rounded-3xl border border-slate-100/50">
                <div>
                    <h1 className="text-2xl font-extrabold text-[#2563EB] tracking-tight">
            ...
            {/* Tab Switching (Internal) */}
             <div className="bg-white rounded-2xl p-1.5 flex w-full max-w-md shadow-sm border border-slate-200 mb-8">
                ...
                <button 
                    ...
                    className={`flex-1 py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all
                    ${activeTab === 'virement' ? 'bg-[#1e3a8a]/10 text-[#1e3a8a] shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}
                >
                ...
                <div className="flex gap-4">
                     <div className="relative flex-1">
                        ...
                        <input 
                            ...
                            className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB] transition-all shadow-sm" 
                        />
                    </div>
                    <button 
                        ...
                        className={`flex items-center gap-2 px-8 py-3 rounded-xl text-white font-bold shadow-lg transition-transform active:scale-95
                             ${activeTab === 'espece' ? 'bg-amber-600 hover:bg-amber-700 shadow-amber-500/20' : 'bg-[#1e3a8a] hover:bg-[#1e40af] shadow-blue-900/20'}
                        `}
                    >
